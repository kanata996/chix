...
- 实际读取到零字节 body：binding 层 no-op；是否报错取决于后续 `RequestValidator`、字段校验或业务规则
- 非法 JSON：`400 invalid_json`
- 非支持的 `Content-Type`：`415 unsupported_media_type`

binding 错误默认不输出 `errors[]` violation 列表；`errors[]` 主要由 `hah` 的请求规则层和字段校验层产出。
