...
本文定义 `reqx/chix` 下一版 binding API 的目标形态，用作重构实施和最终验收的唯一标准。
...
func BindAndValidate(r *http.Request, dst any) error {
	if err := reqx.Bind(r, dst); err != nil {
		return err
	}
...
`reqx` 必须成为 binding 规范面的唯一权威包。`chix` 根包是否 re-export，只是便利层，不是标准本体。
...
`ValueBinder` 和泛型 extractor 更适合保留在 `reqx`，避免根包继续膨胀。
...
1. 为 `reqx` 写一组 adapter case，把 `*http.Request` 构造成 Echo 等价输入
...
如果 `reqx` 最终保留自己的错误包裹类型，验收时可以不要求错误字符串逐字节一致，但下面这些结构化事实必须一致：
