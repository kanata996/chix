...
func TestAdapterMapsPathQueryAndNestedBodyViolations(t *testing.T) {
	type profile struct {
		Name string `json:"name" validate:"required"`
	}
	type input struct {
	...
	rt := chix.New()
	router := chi.NewRouter()
	router.Method(http.MethodPost, "/users/{id}", chix.Handle(rt, chix.Operation[input, struct{}]{
		Method:   http.MethodPost,
		Validate: validatorx.Adapter[input](playgroundvalidator.New(playgroundvalidator.WithRequiredStructEnabled())),
	}, func(_ context.Context, _ *input) (*struct{}, error) {
		t.Fatal("handler should not run")
	...
	if len(envelope.Error.Details) != 3 {
		t.Fatalf("expected 3 violations, got %+v", envelope.Error.Details)
	}
	...
	if violation, ok := got["path:id"]; !ok || violation.Code != "min" {
		t.Fatalf("expected path violation, got %+v", envelope.Error.Details)
	}
	if violation, ok := got["query:limit"]; !ok || violation.Code != "gt" {
		t.Fatalf("expected query violation, got %+v", envelope.Error.Details)
	}
	if violation, ok := got["body:profile.name"]; !ok || violation.Code != "required" {
		t.Fatalf("expected nested body violation, got %+v", envelope.Error.Details)
	}
}
...
func TestAdapterMapsEmbeddedBodyViolations(t *testing.T) {
	type payload struct {
		Name string `json:"name" validate:"required"`
	}
	type input struct {
	...
	violation := envelope.Error.Details[0]
	if violation.Source != "body" || violation.Field != "name" || violation.Code != "required" {
		t.Fatalf("unexpected violation: %+v", violation)
	}
}

func TestAdapterPanicsOnInvalidInputSchema(t *testing.T) {
	type profile struct {
	...
	_ = validatorx.Adapter[input](playgroundvalidator.New(playgroundvalidator.WithRequiredStructEnabled()))
}
