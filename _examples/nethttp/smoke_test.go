...
func TestNetHTTPIntegrationSmoke(t *testing.T) {
	router := newRouter()
	...
	tests := []struct {
		...
		},
		{
			...
			assertions: func(t *testing.T, rr *httptest.ResponseRecorder) {
				t.Helper()
				assertErrorResponse(t, rr, http.StatusUnprocessableEntity, "invalid_request", "request contains invalid fields")
				assertFirstDetail(t, rr, "display_name", "min_length", "must be at least 3 characters")
			},
		},
		{
			...
			assertions: func(t *testing.T, rr *httptest.ResponseRecorder) {
				t.Helper()
				assertErrorResponse(t, rr, http.StatusUnprocessableEntity, "invalid_request", "request contains invalid fields")
				assertFirstDetail(t, rr, "profile", "required", "display_name or timezone is required")
			},
		},
		{
		...
		},
		{
			...
			assertions: func(t *testing.T, rr *httptest.ResponseRecorder) {
				t.Helper()
				assertErrorResponse(t, rr, http.StatusNotAcceptable, "unsupported_format", "format must be json or csv")
			},
		},
		{
		...
		},
		{
			...
			assertions: func(t *testing.T, rr *httptest.ResponseRecorder) {
				t.Helper()
				assertErrorResponse(t, rr, http.StatusGone, "invite_expired", "invite has expired")
			},
		},
		{
			...
			assertions: func(t *testing.T, rr *httptest.ResponseRecorder) {
				t.Helper()
				assertErrorResponse(t, rr, http.StatusConflict, "user_already_suspended", "user already suspended")
			},
		},
		{
		...
		},
		{
			...
			assertions: func(t *testing.T, rr *httptest.ResponseRecorder) {
				t.Helper()
				assertErrorResponse(t, rr, http.StatusBadGateway, "upstream_unavailable", "upstream unavailable")
			},
		},
		{
		...
		},
		{
			...
			assertions: func(t *testing.T, rr *httptest.ResponseRecorder) {
				t.Helper()
				assertErrorResponse(t, rr, http.StatusMethodNotAllowed, "method_not_allowed", "method not allowed")
			},
		},
		{
			...
			assertions: func(t *testing.T, rr *httptest.ResponseRecorder) {
				t.Helper()
				assertErrorResponse(t, rr, http.StatusUnauthorized, "unauthorized", "missing authorization token")
			},
		},
		{
...
func TestNetHTTPPartialPanicsWhenWriteFails(t *testing.T) {
	router := newRouter()
